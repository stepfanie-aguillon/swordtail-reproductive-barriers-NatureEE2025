...
focal<-read.csv(file="chr7_chr14_focal_peak_region.txt",sep="\t",head=TRUE)
...
results<-rbind(results,cbind("chr7_peak",as.character(colnames(data[i])),mean(combined[,1])/2,mean(combined[,2])/2,exp1,exp2,exp3,exp4,exp5,exp6,exp7,exp8,exp9,obs1,obs2,obs3,obs4,obs5,obs6,obs7,obs8,obs9,total,chival))

}

whole_genome<-rbind(whole_genome,results)

write.table(whole_genome,file=paste("results_chisq_chr7_genome_scan_",file,sep=""),sep="\t",row.names=FALSE,col.names=c("chr1_site1","chr2_site2","a1","a2","exp1","exp2","exp3","exp4","exp5","exp6","exp7","exp8","exp9","MMmm","MMmb","MMbb","MBmm","MBmb","MBbb","BBmm","BBmb","BBbb","total","chisqstat"),quote=FALSE)
